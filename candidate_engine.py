...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False,   # ⚠️ allow_origins=["*"]와 True 조합 금지
    allow_methods=["*"],       # GET → * 로 변경 (POST 포함)
    allow_headers=["*"],
)
...
SECTOR_LEADERS = {
    ...
    "042700",   # 한미반도체
    "000660",   # SK하이닉스
    "336260",   # (구) — 잘못된 코드, 두산퓨얼셀임. 호환을 위해 유지
    "007660",   # 이수페타시스 (진짜 코드)
    "114120",   # 주성엔지니어링
    "058470",   # 리노공업
    "039030",   # 이오테크닉스
...
async def scan_candidates(limit: int = 10) -> tuple[list[dict], dict]:
    ...
    for stock in CANDIDATE_UNIVERSE:
        ...
        candidates.append({
            ...
            "scan_time":    datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),  # ★ Time Intelligence v1.0
            # 판정
            "score":        total,
            "grade":        _grade(total),
            "status":       _status(data),
...
# ═══════════════════════════════════════════════════════════════════════
# § 8-B.  진단 엔드포인트: GET /diag
# ═══════════════════════════════════════════════════════════════════════

@app.get("/diag")
async def diag(code: str = "005930"):
    """
    실시간 진단 — 토큰 발급 + KIS API 응답을 직접 확인.
    
    Query Params:
      code: 조회할 종목코드 (default: 삼성전자 005930)

    이 엔드포인트로 mock 폴백 원인을 정확히 파악합니다.
    """
    result = {
        "checked_at": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
        "env": {
            "KIS_MODE":            KIS_MODE,
            "KIS_APP_KEY_set":     bool(KIS_APP_KEY),
            "KIS_APP_KEY_len":     len(KIS_APP_KEY) if KIS_APP_KEY else 0,
            "KIS_APP_SECRET_set":  bool(KIS_APP_SECRET),
            "KIS_APP_SECRET_len":  len(KIS_APP_SECRET) if KIS_APP_SECRET else 0,
        },
        "token": {"status": "not_attempted", "preview": "", "error": ""},
        "api_call": {"status": "not_attempted", "code": code, "raw_output": None, "error": ""},
        "verdict": "",
    }

    # ── 1. 토큰 발급 시도
    if KIS_MODE != "real":
        result["verdict"] = "❌ KIS_MODE != 'real' — Railway 환경변수에서 KIS_MODE=real 설정 필요"
        return result

    if not KIS_APP_KEY or not KIS_APP_SECRET:
        result["verdict"] = "❌ KIS_APP_KEY 또는 KIS_APP_SECRET 누락 — Railway 환경변수 설정 필요"
        return result
    ...
    try:
        ...
        if token:
            ...
            result["token"]["preview"] = token[:20] + "..." if len(token) > 20 else token
        else:
            result["token"]["status"] = "empty"
            result["verdict"] = "❌ 토큰이 빈 값으로 발급됨"
            return result
    except Exception as e:
        result["token"]["status"] = "failed"
        result["token"]["error"]  = f"{type(e).__name__}: {str(e)[:200]}"
        result["verdict"]         = "❌ 토큰 발급 실패 — APP_KEY/SECRET 값이 잘못됐거나 KIS 서버 문제"
        return result
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            r = await client.get(url, headers=headers, params=params)
        result["api_call"]["http_status"] = r.status_code
        if r.status_code != 200:
            result["api_call"]["status"]    = "non_200"
            result["api_call"]["body"]      = r.text[:500]
            result["verdict"]               = f"❌ KIS API HTTP {r.status_code} — 응답 body 확인 필요"
            return result

        body = r.json()
        result["api_call"]["status"]     = "success"
        result["api_call"]["rt_cd"]      = body.get("rt_cd")
        result["api_call"]["msg_cd"]     = body.get("msg_cd")
        result["api_call"]["msg1"]       = body.get("msg1")
        output = body.get("output", {}) or {}
        result["api_call"]["raw_output"] = {
        ...
        if not output:
            result["verdict"] = "❌ KIS API output 빈 응답 — rt_cd/msg1 확인"
        elif output.get("stck_prpr") in (None, "", "0"):
            result["verdict"] = f"⚠️ 가격 0/빈값 — KIS rt_cd: {body.get('rt_cd')}, msg: {body.get('msg1')}"
        else:
            result["verdict"] = f"✅ 정상! {code} 현재가 = {output.get('stck_prpr')}원"
    except Exception as e:
        result["api_call"]["status"] = "exception"
        result["api_call"]["error"]  = f"{type(e).__name__}: {str(e)[:200]}"
        result["verdict"]            = "❌ KIS API 호출 중 예외 발생"
